...
function parse_VtHeadingPair(blob) {
  ...
  var headerParts2 = parse_TypedPropertyValue(blob, VT_I4);
  return [headingString, headerParts2];
}
...
// standalone/.attendance-build.ts
var identityHeaders = {
  amCode: ["am code", "amcode", "am id", "am identifier", "account manager code"],
  amNo: ["am no", "amno", "am number", "am no.", "am phone", "am mobile", "am contact", "mobile no", "mobile number", "phone no", "phone number", "contact no", "contact number", "number", "phone", "mobile"],
  amName: ["am name", "amname", "account manager", "account manager name", "am", "name", "employee name", "person name", "resource name"],
  vendor: ["vendor", "vendor name", "agency", "partner", "supplier", "contractor"]
};
var dateHeaders = ["date", "attendance date", "att date", "work date", "duty date", "event date"];
var inHeaders = ["in", "in time", "check in", "checkin", "punch in", "login", "entry", "entry time", "start time"];
var outHeaders = ["out", "out time", "check out", "checkout", "punch out", "logout", "exit", "exit time", "end time"];
var monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
var absenceTokens = /* @__PURE__ */ new Set([
  "-", "--", "—", "0", "a", "ab", "absent", "h", "holiday", "l", "leave", "na", "n/a", "nil", "no",
  "no attendance", "no punch", "not present", "off", "w/o", "week off", "weekly off", "wo"
]);
function normalized(value) {
  return value.toLocaleLowerCase("en-IN").replace(/[_.]+/g, " ").replace(/[^\p{L}\p{N}+/-]+/gu, " ").replace(/\s+/g, " ").trim();
}
function labelledTimes(inValue, outValue) {
  return `IN ${inValue || "not recorded"}, OUT ${outValue || "not recorded"}`;
}
function comparisonDetail(row) {
  const payment = labelledTimes(row.paymentIn, row.paymentOut);
  const rm = labelledTimes(row.rmIn, row.rmOut);
  if (row.status === "Missing in RM") return `${row.date}: Payment recorded ${payment}. RM has no attendance entry for this date.`;
  if (row.status === "Extra in RM") return `${row.date}: RM recorded ${rm}. Payment has no attendance entry for this date.`;
  if (row.status === "IN/OUT mismatch") return `${row.date}: the times differ. Payment — ${payment}; RM — ${rm}.`;
  if (row.status === "Needs review") return `${row.date}: this date-specific record could not be reconciled safely.`;
  return `${row.date}: both files agree. Payment — ${payment}; RM — ${rm}.`;
}
function clean(value) {
  if (value instanceof Date && !Number.isNaN(value.getTime())) return value.toISOString();
  return String(value ?? "").replace(/\u00a0/g, " ").trim();
}
function sourceError(sourceLabel, message) {
  return new Error(`${sourceLabel}: ${message}`);
}
function daysInMonth(year, month) {
  return new Date(Date.UTC(year, month, 0)).getUTCDate();
}
function parseDatePart(input, fallbackYear) {
  if (input instanceof Date && !Number.isNaN(input.getTime())) {
    const year2 = input.getUTCFullYear();
    const month2 = input.getUTCMonth() + 1;
    const day2 = input.getUTCDate();
    return { key: `${year2}-${String(month2).padStart(2, "0")}-${String(day2).padStart(2, "0")}`, display: `${String(day2).padStart(2, "0")}-${monthNames[month2 - 1]}-${year2}` };
  }
  const value = clean(input).replace(/\s+/g, " ");
  let day = 0;
  let month = 0;
  let year = 0;
  const iso = value.match(/(?:^|\D)(\d{4})[-/](\d{1,2})[-/](\d{1,2})(?:\D|$)/);
  const namedDayFirst = value.match(/(?:^|\D)(\d{1,2})[\s/.\-]([A-Za-z]{3,9})[\s/.\-](\d{2}|\d{4})(?:\D|$)/);
  const namedMonthFirst = value.match(/(?:^|\D)([A-Za-z]{3,9})[\s/.\-](\d{1,2})(?:,)?[\s/.\-]+(\d{2}|\d{4})(?:\D|$)/);
  const numeric = value.match(/(?:^|\D)(\d{1,2})[/.-](\d{1,2})[/.-](\d{2}|\d{4})(?:\D|$)/);
  const namedDayFirstWithoutYear = fallbackYear ? value.match(/(?:^|\D)(\d{1,2})[\s/.\-]([A-Za-z]{3,9})(?:\D|$)/) : null;
  const namedMonthFirstWithoutYear = fallbackYear ? value.match(/(?:^|\D)([A-Za-z]{3,9})[\s/.\-](\d{1,2})(?:\D|$)/) : null;
  const numericWithoutYear = fallbackYear ? value.match(/(?:^|\D)(\d{1,2})[/.-](\d{1,2})(?:\D|$)/) : null;
  if (iso) {
    year = Number(iso[1]);
    month = Number(iso[2]);
    day = Number(iso[3]);
  } else if (namedDayFirst) {
    day = Number(namedDayFirst[1]);
    month = monthNames.findIndex((name) => normalized(name) === normalized(namedDayFirst[2]).slice(0, 3)) + 1;
    year = Number(namedDayFirst[3]);
  } else if (namedMonthFirst) {
    month = monthNames.findIndex((name) => normalized(name) === normalized(namedMonthFirst[1]).slice(0, 3)) + 1;
    day = Number(namedMonthFirst[2]);
    year = Number(namedMonthFirst[3]);
  } else if (numeric) {
    day = Number(numeric[1]);
    month = Number(numeric[2]);
    year = Number(numeric[3]);
  } else if (namedDayFirstWithoutYear) {
    day = Number(namedDayFirstWithoutYear[1]);
    month = monthNames.findIndex((name) => normalized(name) === normalized(namedDayFirstWithoutYear[2]).slice(0, 3)) + 1;
    year = fallbackYear;
  } else if (namedMonthFirstWithoutYear) {
    month = monthNames.findIndex((name) => normalized(name) === normalized(namedMonthFirstWithoutYear[1]).slice(0, 3)) + 1;
    day = Number(namedMonthFirstWithoutYear[2]);
    year = fallbackYear;
  } else if (numericWithoutYear) {
    day = Number(numericWithoutYear[1]);
    month = Number(numericWithoutYear[2]);
    year = fallbackYear;
  ...
  if (year < 2e3 || year > 2100 || month < 1 || month > 12 || day < 1 || day > daysInMonth(year, month)) return null;
  const key = `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
  return { key, display: `${String(day).padStart(2, "0")}-${monthNames[month - 1]}-${year}` };
}
function modeFromText(value) {
  const key = normalized(value);
  if (inHeaders.includes(key)) return "in";
  if (outHeaders.includes(key)) return "out";
  return null;
}
function parseDateHeader(header, fallbackYear) {
  const value = clean(header);
  const suffix = value.match(/^(.+?)(?:\s*[-_|:]\s*|\s+)(check\s*in|check\s*out|punch\s*in|punch\s*out|in\s*time|out\s*time|entry\s*time|exit\s*time|start\s*time|end\s*time|login|logout|in|out)$/i);
  const prefix = value.match(/^(check\s*in|check\s*out|punch\s*in|punch\s*out|in\s*time|out\s*time|entry\s*time|exit\s*time|start\s*time|end\s*time|login|logout|in|out)(?:\s*[-_|:]\s*|\s+)(.+)$/i);
  const dateText = suffix?.[1] ?? prefix?.[2];
  const modeText = suffix?.[2] ?? prefix?.[1];
  if (!dateText || !modeText) return null;
  const parsedDate = parseDatePart(dateText, fallbackYear);
  const mode = modeFromText(modeText);
  return parsedDate && mode ? { dateKey: parsedDate.key, display: parsedDate.display, mode } : null;
}
function parseAttendanceCell(value) {
  const source = clean(value);
  if (!source) return { kind: "empty", normalized: "" };
  const token = normalized(source);
  if (!token || absenceTokens.has(token)) return { kind: "absent", normalized: "" };
  const numeric = Number(source);
  if (/^\d*\.?\d+$/.test(source) && Number.isFinite(numeric) && numeric >= 0 && numeric < 1) {
    const minutes = Math.round(numeric * 24 * 60) % (24 * 60);
    return { kind: "time", normalized: `${String(Math.floor(minutes / 60)).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")}` };
  }
  const time = source.match(/(?:^|\s)(\d{1,2})(?::(\d{1,2}))(?::\d{1,2})?\s*(AM|PM)?(?:\s|$)/i);
  if (time) {
    ...
    if (minute < 60 && hour <= 24) {
      ...
      if (hour < 24) return { kind: "time", normalized: `${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}` };
    }
  }
  return { kind: "unknown", normalized: token };
}
function attendanceRecordValues(inValue, outValue, rowNumber) {
  const inCell = parseAttendanceCell(inValue);
  const outCell = parseAttendanceCell(outValue);
  const hasTime = inCell.kind === "time" || outCell.kind === "time";
  if (!hasTime) return null;
  const concerns = [];
  if (inCell.kind === "unknown" || outCell.kind === "unknown") concerns.push(`An IN/OUT value at row ${rowNumber} is not a recognizable time.`);
  if (inCell.kind === "absent" && outCell.kind === "time" || outCell.kind === "absent" && inCell.kind === "time") concerns.push(`Row ${rowNumber} mixes an absence marker with a recorded time.`);
  return { normalizedIn: inCell.normalized, normalizedOut: outCell.normalized, sourceConflict: concerns.join(" ") };
}
function expandMergedCells(sheet, matrix) {
  const output = matrix.map((row) => [...row]);
  for (const merge of sheet["!merges"] ?? []) {
    const anchor = output[merge.s.r]?.[merge.s.c] ?? sheet[utils.encode_cell(merge.s)]?.w ?? sheet[utils.encode_cell(merge.s)]?.v ?? "";
    for (let rowIndex = merge.s.r; rowIndex <= merge.e.r; rowIndex += 1) {
      if (!output[rowIndex]) output[rowIndex] = [];
      for (let columnIndex = merge.s.c; columnIndex <= merge.e.c; columnIndex += 1) output[rowIndex][columnIndex] = anchor;
    }
  }
  return output;
}
function headerParts(matrix, start, end, column) {
  return matrix.slice(start, end + 1).map((row) => clean(row[column])).filter(Boolean);
}
function headerAliasScore(parts, aliases) {
  let score = 0;
  parts.forEach((part, index) => {
    const value = normalized(part);
    if (aliases.includes(value)) score = Math.max(score, 100 + index);
    else if (aliases.some((alias) => value.includes(alias) && alias.length >= 4)) score = Math.max(score, 55 + index);
  });
  const combined = normalized(parts.join(" "));
  if (aliases.includes(combined)) score = Math.max(score, 90);
  return score;
}
function resolveIdentityColumns(matrix, start, end, width) {
  const result = {};
  for (const key of Object.keys(identityHeaders)) {
    const scored = Array.from({ length: width }, (_, column) => ({ column, score: headerAliasScore(headerParts(matrix, start, end, column), identityHeaders[key]) })).filter((item) => item.score > 0).sort((a, b) => b.score - a.score);
    if (scored.length && (scored.length === 1 || scored[0].score > scored[1].score)) result[key] = scored[0].column;
  }
  return result;
}
function resolveGenericColumn(matrix, start, end, width, aliases) {
  const scored = Array.from({ length: width }, (_, column) => ({ column, score: headerAliasScore(headerParts(matrix, start, end, column), aliases) })).filter((item) => item.score > 0).sort((a, b) => b.score - a.score);
  return scored.length && (scored.length === 1 || scored[0].score > scored[1].score) ? scored[0].column : void 0;
}
function resolveExactColumn(matrix, start, end, width, aliases) {
  const scored = Array.from({ length: width }, (_, column) => {
    const parts = headerParts(matrix, start, end, column).map(normalized);
    const combined = normalized(parts.join(" "));
    const score = parts.some((part) => aliases.includes(part)) ? 100 : aliases.includes(combined) ? 90 : 0;
    return { column, score };
  }).filter((item) => item.score > 0).sort((a, b) => b.score - a.score);
  return scored.length && (scored.length === 1 || scored[0].score > scored[1].score) ? scored[0].column : void 0;
}
function parseWideDateParts(parts, fallbackYear) {
  for (const part of parts) {
    const parsed = parseDateHeader(part, fallbackYear);
    if (parsed) return parsed;
  }
  const combined = parseDateHeader(parts.join(" "), fallbackYear);
  if (combined) return combined;
  const date = parts.map((part) => parseDatePart(part, fallbackYear)).find(Boolean);
  const mode = parts.map(modeFromText).find(Boolean);
  return date && mode ? { dateKey: date.key, display: date.display, mode } : null;
}
function inferContextYear(matrix) {
  const years = /* @__PURE__ */ new Map();
  matrix.forEach((row) => row.forEach((cell) => {
    const parsed = parseDatePart(cell);
    if (parsed) {
      const year = Number(parsed.key.slice(0, 4));
      years.set(year, (years.get(year) ?? 0) + 1);
    }
  }));
  const ranked = [...years.entries()].sort((left, right) => right[1] - left[1]);
  if (ranked.length === 1) return ranked[0][0];
  if (ranked[0] && ranked[0][1] >= 5 && ranked[0][1] >= (ranked[1]?.[1] ?? 0) * 10) return ranked[0][0];
  return void 0;
}
function detectLayout(matrix, fallbackYear) {
  const maxHeaderRow = Math.min(matrix.length, 30);
  const width = Math.max(0, ...matrix.slice(0, maxHeaderRow).map((row) => row.length));
  const candidates = [];
  for (let end = 0; end < maxHeaderRow; end += 1) {
    for (let depth = 1; depth <= 3; depth += 1) {
      const start = end - depth + 1;
      if (start < 0) continue;
      const identities = resolveIdentityColumns(matrix, start, end, width);
      const identityCount = Object.keys(identities).length;
      const hasStableIdentity = identities.amCode !== void 0 || identities.amNo !== void 0;
      const hasDescriptiveIdentity = identities.amName !== void 0 || identities.vendor !== void 0;
      if (identityCount < 2 || !hasStableIdentity && !(identities.amName !== void 0 && identities.vendor !== void 0) || !hasDescriptiveIdentity) continue;
      const wideDates = /* @__PURE__ */ new Map();
      for (let column = 0; column < width; column += 1) {
        const parsed = parseWideDateParts(headerParts(matrix, start, end, column), fallbackYear);
        if (!parsed) continue;
        const current = wideDates.get(parsed.dateKey) ?? { display: parsed.display };
        if (parsed.mode === "in" && current.inColumn === void 0) current.inColumn = column;
        if (parsed.mode === "out" && current.outColumn === void 0) current.outColumn = column;
        wideDates.set(parsed.dateKey, current);
      }
      const dateColumn = resolveExactColumn(matrix, start, end, width, dateHeaders);
      const generalIn = resolveGenericColumn(matrix, start, end, width, inHeaders);
      const generalOut = resolveGenericColumn(matrix, start, end, width, outHeaders);
      const longColumns = !wideDates.size && dateColumn !== void 0 && (generalIn !== void 0 || generalOut !== void 0) ? { dateColumn, inColumn: generalIn, outColumn: generalOut } : void 0;
      if (!wideDates.size && !longColumns) continue;
      const pairedDates = [...wideDates.values()].filter((item) => item.inColumn !== void 0 && item.outColumn !== void 0).length;
      const score = identityCount * 100 + wideDates.size * 20 + pairedDates * 12 + (longColumns ? 35 : 0) - start;
      candidates.push({ headerStart: start, headerEnd: end, identityColumns: identities, wideDates, longColumns, score });
    }
  }
  candidates.sort((a, b) => b.score - a.score || a.headerEnd - b.headerEnd);
  return candidates[0] ?? null;
}
function profileCompatible(profile, values) {
  if (profile.normCode && values.normCode && profile.normCode !== values.normCode) return false;
  if (profile.normNo && values.normNo && profile.normNo !== values.normNo) return false;
  if (profile.normName && values.normName && profile.normName !== values.normName) return false;
  if (profile.normVendor && values.normVendor && profile.normVendor !== values.normVendor) return false;
  return true;
}
function mergeRecord(profile, record) {
  const existing = profile.records.get(record.dateKey);
  if (!existing) {
    profile.records.set(record.dateKey, record);
    return;
  }
  const inConflict = existing.normalizedIn && record.normalizedIn && existing.normalizedIn !== record.normalizedIn;
  const outConflict = existing.normalizedOut && record.normalizedOut && existing.normalizedOut !== record.normalizedOut;
  profile.records.set(record.dateKey, {
    ...existing,
    inValue: existing.inValue || record.inValue,
    outValue: existing.outValue || record.outValue,
    normalizedIn: existing.normalizedIn || record.normalizedIn,
    normalizedOut: existing.normalizedOut || record.normalizedOut,
    sourceConflict: inConflict || outConflict ? `Conflicting attendance values occur within this source for ${record.date}.` : existing.sourceConflict || record.sourceConflict
  });
}
function getIndexValues(index, key) {
  return key ? [...index.get(key) ?? []] : [];
}
async function readAttendanceFile(file, sourceLabel) {
  if (!/\.(xlsx|xls|csv)$/i.test(file.name)) throw sourceError(sourceLabel, "upload an .xlsx, .xls, or .csv file.");
  let workbook;
  try {
    workbook = readSync(await file.arrayBuffer(), { type: "array", cellDates: true, cellFormula: true });
  } catch {
    throw sourceError(sourceLabel, "the workbook could not be opened. Confirm that it is not damaged or password-protected.");
  }
  const detected = workbook.SheetNames.map((name) => {
    const sheet = workbook.Sheets[name];
    const rawMatrix = utils.sheet_to_json(sheet, { header: 1, defval: "", raw: false, blankrows: true });
    const matrix = expandMergedCells(sheet, rawMatrix);
    const populated = matrix.some((row) => row.some((cell) => clean(cell) !== ""));
    const contextYear = populated ? inferContextYear(matrix) : void 0;
    return { name, sheet, matrix, contextYear, layout: populated ? detectLayout(matrix, contextYear) : null };
  }).filter((item) => item.layout !== null);
  if (!detected.length) {
    throw sourceError(sourceLabel, "this workbook layout could not be mapped safely. Do not edit the source file; the importer needs support for this layout.");
  }
  const profiles = [];
  const codeIndex = /* @__PURE__ */ new Map();
  const noIndex = /* @__PURE__ */ new Map();
  const nameVendorIndex = /* @__PURE__ */ new Map();
  const dates = /* @__PURE__ */ new Map();
  const identityFields = /* @__PURE__ */ new Set();
  const notes = /* @__PURE__ */ new Set();
  let mergedRangesExpanded = 0;
  let formulaCellsRead = 0;
  let dataRows = 0;
  function indexProfile(index, key, profile) {
    if (!key) return;
    if (!index.has(key)) index.set(key, /* @__PURE__ */ new Set());
    index.get(key).add(profile);
  }
  detected.forEach(({ name, sheet, matrix, contextYear, layout }) => {
    if (!layout) return;
    mergedRangesExpanded += sheet["!merges"]?.length ?? 0;
    formulaCellsRead += Object.entries(sheet).filter(([address, cell]) => !address.startsWith("!") && typeof cell === "object" && cell !== null && "f" in cell).length;
    Object.keys(layout.identityColumns).forEach((field) => identityFields.add(field));
    if (layout.wideDates.size) notes.add("Wide date columns normalized");
    if (layout.longColumns) notes.add("Row-wise date columns normalized");
    if (contextYear) notes.add(`Workbook date context resolved to ${contextYear}`);
    matrix.slice(layout.headerEnd + 1).forEach((cells, relativeIndex) => {
      const rowNumber = layout.headerEnd + relativeIndex + 2;
      const readIdentity = (key) => {
        const column = layout.identityColumns[key];
        return column === void 0 ? "" : clean(cells[column]);
      };
      const amCode = readIdentity("amCode");
      const amNo = readIdentity("amNo");
      const amName = readIdentity("amName");
      const vendor = readIdentity("vendor");
      const normCode = normalized(amCode);
      const normNo = normalized(amNo);
      const normName = normalized(amName);
      const normVendor = normalized(vendor);
      const extracted = [];
      for (const [dateKey, columns] of layout.wideDates) {
        const inValue = columns.inColumn === void 0 ? "" : clean(cells[columns.inColumn]);
        const outValue = columns.outColumn === void 0 ? "" : clean(cells[columns.outColumn]);
        const values = attendanceRecordValues(inValue, outValue, rowNumber);
        if (!values) continue;
        extracted.push({ date: columns.display, dateKey, inValue, outValue, ...values, sourceRow: rowNumber });
      }
      if (layout.longColumns) {
        const inValue = layout.longColumns.inColumn === void 0 ? "" : clean(cells[layout.longColumns.inColumn]);
        const outValue = layout.longColumns.outColumn === void 0 ? "" : clean(cells[layout.longColumns.outColumn]);
        const values = attendanceRecordValues(inValue, outValue, rowNumber);
        if (values) {
          const rawDate = cells[layout.longColumns.dateColumn];
          const parsedDate = parseDatePart(rawDate);
          extracted.push({
            date: parsedDate?.display ?? (clean(rawDate) || "Date not provided"),
            dateKey: parsedDate?.key ?? `review:${normalized(name)}:${rowNumber}`,
            inValue,
            outValue,
            normalizedIn: values.normalizedIn,
            normalizedOut: values.normalizedOut,
            sourceRow: rowNumber,
            sourceConflict: [values.sourceConflict, parsedDate ? "" : `The attendance date at row ${rowNumber} could not be interpreted safely.`].filter(Boolean).join(" ")
          });
        }
      }
      if (!extracted.length && !normCode && !normNo && !normName && !normVendor) return;
      dataRows += 1;
      const nvKey = normName && normVendor ? `${normName}|${normVendor}` : "";
      const codeCandidates = getIndexValues(codeIndex, normCode);
      const noCandidates = getIndexValues(noIndex, normNo);
      const strongCandidates = /* @__PURE__ */ new Set([
        ...codeCandidates,
        ...noCandidates
      ]);
      const descriptiveCandidates = !normCode && !normNo ? new Set(getIndexValues(nameVendorIndex, nvKey)) : /* @__PURE__ */ new Set();
      const candidates = strongCandidates.size ? strongCandidates : descriptiveCandidates;
      const compatible = [...candidates].filter((profile2) => profileCompatible(profile2, { normCode, normNo, normName, normVendor }));
      let profile = compatible.length === 1 ? compatible[0] : void 0;
      const identityIssue = !normCode && !normNo && !(normName && normVendor) ? `Row ${rowNumber} does not contain enough identity fields for deterministic matching.` : compatible.length > 1 || candidates.size > 0 && compatible.length === 0 && !(normCode && codeCandidates.length === 0) ? `Conflicting or duplicate AM identifiers occur around row ${rowNumber}.` : "";
      if (!profile) {
        profile = {
          id: `${normalized(sourceLabel)}:${normalized(name)}:${rowNumber}`,
          amCode,
          amNo,
          amName,
          vendor,
        ...
        profile.amCode || (profile.amCode = amCode);
        profile.amNo || (profile.amNo = amNo);
        profile.amName || (profile.amName = amName);
        profile.vendor || (profile.vendor = vendor);
        profile.normCode || (profile.normCode = normCode);
        profile.normNo || (profile.normNo = normNo);
        profile.normName || (profile.normName = normName);
        profile.normVendor || (profile.normVendor = normVendor);
        profile.identityIssue || (profile.identityIssue = identityIssue);
      }
      indexProfile(codeIndex, normCode, profile);
      indexProfile(noIndex, normNo, profile);
      indexProfile(nameVendorIndex, nvKey, profile);
      extracted.forEach((record) => {
        dates.set(record.dateKey, record.date);
        mergeRecord(profile, record);
      });
    });
  });
  const attendanceRecords = profiles.reduce((sum, profile) => sum + profile.records.size, 0);
  if (!attendanceRecords) throw sourceError(sourceLabel, "attendance headers were found, but no non-empty IN/OUT values were found below them.");
  if (mergedRangesExpanded) notes.add(`${mergedRangesExpanded} merged range${mergedRangesExpanded === 1 ? "" : "s"} expanded in memory`);
  if (formulaCellsRead) notes.add(`${formulaCellsRead} formula cell${formulaCellsRead === 1 ? "" : "s"} read from stored workbook values`);
  if (detected.length > 1) notes.add(`${detected.length} attendance worksheets combined`);
  const issueCount = profiles.filter((profile) => profile.identityIssue || [...profile.records.values()].some((record) => record.sourceConflict)).length;
  if (issueCount) notes.add(`${issueCount} ambiguous source identit${issueCount === 1 ? "y" : "ies"} excluded from automatic reconciliation`);
  return {
    profiles,
    audit: {
      fileName: file.name,
      sheetName: detected.map((item) => item.name).join(", "),
      dataRows,
      attendanceRecords,
      dates: [...dates.entries()].filter(([key]) => !key.startsWith("review:")).sort(([left], [right]) => left.localeCompare(right)).map(([, display]) => display),
      mergedRangesExpanded,
      formulaCellsRead,
      identityFields: [...identityFields].map((field) => field === "amCode" ? "AM Code" : field === "amNo" ? "AM No" : field === "amName" ? "AM Name" : "Vendor"),
      notes: [...notes]
    }
  };
}
function createProfileIndex(profiles, key) {
  const index = /* @__PURE__ */ new Map();
  profiles.forEach((profile) => {
    const value = key === "nameVendor" ? profile.normName && profile.normVendor ? `${profile.normName}|${profile.normVendor}` : "" : profile[key];
    if (!value) return;
    if (!index.has(value)) index.set(value, []);
    index.get(value).push(profile);
  });
  return index;
}
function strongIdentityConflict(payment, rm) {
  if (payment.normCode && rm.normCode && payment.normCode !== rm.normCode) return "AM Code differs between otherwise matching records.";
  if (payment.normNo && rm.normNo && payment.normNo !== rm.normNo) return "AM Number differs between otherwise matching records.";
  return "";
}
function rowIdentity(payment, rm) {
  return {
    amCode: payment?.amCode || rm?.amCode || "",
    amNo: payment?.amNo || rm?.amNo || "",
    amName: payment?.amName || rm?.amName || "",
    vendor: payment?.vendor || rm?.vendor || ""
  };
}
function buildMatchProposals(payment, rm) {
  const rmByCode = createProfileIndex(rm.profiles, "normCode");
  const rmByNo = createProfileIndex(rm.profiles, "normNo");
  const rmByNameVendor = createProfileIndex(rm.profiles, "nameVendor");
  const proposals = payment.profiles.map((payProfile) => {
    const scored = /* @__PURE__ */ new Map();
    const add = (profiles, score, basis) => profiles.forEach((profile) => {
      const existing = scored.get(profile.id);
      if (!existing || score > existing.score) scored.set(profile.id, { profile, score, basis });
    });
    if (payProfile.normCode) add(rmByCode.get(payProfile.normCode) ?? [], 300, "AM Code");
    if (payProfile.normNo) add(rmByNo.get(payProfile.normNo) ?? [], 250, "AM No");
    if (payProfile.normName && payProfile.normVendor) add(rmByNameVendor.get(`${payProfile.normName}|${payProfile.normVendor}`) ?? [], 100, "AM Name + Vendor");
    const ordered = [...scored.values()].sort((a, b) => b.score - a.score);
    const top = ordered[0];
    const tied = top ? ordered.filter((item) => item.score === top.score) : [];
    const conflict = top ? strongIdentityConflict(payProfile, top.profile) : "";
    return {
      payment: payProfile,
      candidateIds: ordered.map((item) => item.profile.id),
      rm: top && tied.length === 1 && !conflict ? top.profile : void 0,
      score: top?.score ?? 0,
      basis: top?.basis ?? "Unmatched",
      reason: payProfile.identityIssue || (tied.length > 1 ? `Multiple RM identities match the same ${top?.basis ?? "available identifiers"}.` : conflict)
    };
  });
  const rmProposalCounts = /* @__PURE__ */ new Map();
  proposals.forEach((proposal) => proposal.rm && rmProposalCounts.set(proposal.rm.id, (rmProposalCounts.get(proposal.rm.id) ?? 0) + 1));
  const accepted = [...new Set(proposals.flatMap((proposal) => proposal.rm ? [proposal.rm.id] : []))].flatMap((rmId) => {
    const eligible = proposals.filter((proposal) => proposal.rm?.id === rmId && !proposal.reason).sort((left, right) => right.score - left.score);
    if (!eligible.length) return [];
    const strongest = eligible.filter((proposal) => proposal.score === eligible[0].score);
    return strongest.length === 1 ? strongest : [];
  });
  return { proposals, rmProposalCounts, accepted };
}
function hasSourceConflict(profile) {
  return [...profile.records.values()].some((record) => Boolean(record.sourceConflict));
}
function compareData(payment, rm) {
  const { proposals, rmProposalCounts, accepted } = buildMatchProposals(payment, rm);
  const acceptedPaymentIds = new Set(accepted.map((proposal) => proposal.payment.id));
  const matchedRmIds = new Set(accepted.map((proposal) => proposal.rm.id));
  const reviewRmIds = new Set(proposals.filter((proposal) => proposal.reason || proposal.rm && rmProposalCounts.get(proposal.rm.id) > 1).flatMap((proposal) => proposal.candidateIds));
  const rows = [];
  function pushRecord(args) {
    const identity = rowIdentity(args.paymentProfile, args.rmProfile);
    const record = args.paymentRecord ?? args.rmRecord;
    if (!record) return;
    rows.push({
      ...
      paymentIn: args.paymentRecord?.inValue ?? "",
      paymentOut: args.paymentRecord?.outValue ?? "",
      rmIn: args.rmRecord?.inValue ?? "",
      rmOut: args.rmRecord?.outValue ?? "",
      paymentSourceRow: args.paymentRecord?.sourceRow ?? null,
      rmSourceRow: args.rmRecord?.sourceRow ?? null,
      status: args.status,
      matchBasis: args.basis,
      reviewReason: args.reason ?? ""
    });
  }
  accepted.forEach((proposal) => {
    const rmProfile = proposal.rm;
    const dateKeys = /* @__PURE__ */ new Set([...proposal.payment.records.keys(), ...rmProfile.records.keys()]);
    [...dateKeys].forEach((dateKey) => {
      const payRecord = proposal.payment.records.get(dateKey);
      const rmRecord = rmProfile.records.get(dateKey);
      const conflict = proposal.payment.identityIssue || rmProfile.identityIssue || payRecord?.sourceConflict || rmRecord?.sourceConflict;
      if (conflict) return;
      const status = payRecord && rmRecord ? payRecord.normalizedIn === rmRecord.normalizedIn && payRecord.normalizedOut === rmRecord.normalizedOut ? "Matched" : "IN/OUT mismatch" : payRecord ? "Missing in RM" : "Extra in RM";
      pushRecord({ paymentProfile: proposal.payment, rmProfile, paymentRecord: payRecord, rmRecord, status, basis: proposal.basis, reason: conflict, entityKey: `pair:${proposal.payment.id}:${rmProfile.id}` });
    });
  });
  proposals.filter((proposal) => !acceptedPaymentIds.has(proposal.payment.id) && proposal.candidateIds.length === 0 && !proposal.reason && !proposal.payment.identityIssue && !hasSourceConflict(proposal.payment)).forEach((proposal) => {
    proposal.payment.records.forEach((record) => pushRecord({
      paymentProfile: proposal.payment,
      paymentRecord: record,
      status: "Missing in RM",
      basis: "No deterministic RM match",
      entityKey: `payment:${proposal.payment.id}`
    }));
  });
  rm.profiles.filter((profile) => !matchedRmIds.has(profile.id) && !reviewRmIds.has(profile.id) && !profile.identityIssue && !hasSourceConflict(profile)).forEach((profile) => {
    profile.records.forEach((record) => pushRecord({
      rmProfile: profile,
      rmRecord: record,
      status: "Extra in RM",
      basis: "No deterministic Payment match",
      entityKey: `rm:${profile.id}`
    }));
  });
  return rows.sort((left, right) => left.dateKey.localeCompare(right.dateKey) || left.amName.localeCompare(right.amName) || left.amNo.localeCompare(right.amNo)).map((row, index) => ({ ...row, id: String(index + 1) }));
}
function createPayload(payment, rm, rows, googleSheetUrl) {
  const dates = new Set(rows.filter((row) => !row.dateKey.startsWith("review:")).map((row) => row.dateKey));
  const comparedAms = new Set(rows.map((row) => row.entityKey));
  return {
    rows,
    createdAt: (/* @__PURE__ */ new Date()).toISOString(),
    googleSheetUrl: googleSheetUrl || void 0,
    sources: { payment: payment.audit, rm: rm.audit },
    stats: {
      totalAms: comparedAms.size,
      totalAttendance: rows.length,
      matched: rows.filter((row) => row.status === "Matched").length,
      missing: rows.filter((row) => row.status === "Missing in RM").length,
      extra: rows.filter((row) => row.status === "Extra in RM").length,
      mismatched: rows.filter((row) => row.status === "IN/OUT mismatch").length,
      review: rows.filter((row) => row.status === "Needs review").length,
      detectedDates: dates.size,
      paymentAms: payment.profiles.length,
      rmAms: rm.profiles.length,
      paymentAttendance: payment.audit.attendanceRecords,
      rmAttendance: rm.audit.attendanceRecords
    }
  };
}
function exportWorkbook(payload) {
  const comparison = payload.rows.map((row) => ({
    "AM Code": row.amCode,
    "AM No": row.amNo,
    "AM Name": row.amName,
    Vendor: row.vendor,
    Date: row.date,
    "Payment IN": row.paymentIn,
    "Payment OUT": row.paymentOut,
    "RM IN": row.rmIn,
    "RM OUT": row.rmOut,
    "Payment Source Row": row.paymentSourceRow ?? "",
    "RM Source Row": row.rmSourceRow ?? "",
    "Comparison Status": row.status,
    "Date-specific Result": comparisonDetail(row),
    "Match Basis": row.matchBasis,
    "Review Reason": row.reviewReason
  }));
  const summary = [
    { Metric: "Total AMs", Value: payload.stats.totalAms },
    { Metric: "Total attendance records", Value: payload.stats.totalAttendance },
    { Metric: "Matched records", Value: payload.stats.matched },
    { Metric: "Missing in RM", Value: payload.stats.missing },
    { Metric: "RM extra records", Value: payload.stats.extra },
    { Metric: "IN/OUT mismatches", Value: payload.stats.mismatched },
    { Metric: "Detected dates", Value: payload.stats.detectedDates }
  ];
  const audit = [
    { Source: "Payment", File: payload.sources.payment.fileName, Worksheet: payload.sources.payment.sheetName, "AM rows": payload.sources.payment.dataRows, "Attendance records": payload.sources.payment.attendanceRecords, "Detected dates": payload.sources.payment.dates.join(", "), "Identity fields": payload.sources.payment.identityFields.join(", "), "Merged ranges expanded": payload.sources.payment.mergedRangesExpanded, "Formula cells read": payload.sources.payment.formulaCellsRead, Notes: payload.sources.payment.notes.join("; ") },
    { Source: "RM", File: payload.sources.rm.fileName, Worksheet: payload.sources.rm.sheetName, "AM rows": payload.sources.rm.dataRows, "Attendance records": payload.sources.rm.attendanceRecords, "Detected dates": payload.sources.rm.dates.join(", "), "Identity fields": payload.sources.rm.identityFields.join(", "), "Merged ranges expanded": payload.sources.rm.mergedRangesExpanded, "Formula cells read": payload.sources.rm.formulaCellsRead, Notes: payload.sources.rm.notes.join("; ") }
  ];
  const workbook = utils.book_new();
  utils.book_append_sheet(workbook, utils.json_to_sheet(comparison), "Comparison");
  utils.book_append_sheet(workbook, utils.json_to_sheet(summary), "Summary");
  utils.book_append_sheet(workbook, utils.json_to_sheet(audit), "Source Audit");
  writeFileSync(workbook, `AM_Attendance_Comparison_${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.xlsx`);
}
var STORAGE_KEY = "am-attendance-comparison-v6";
export {
  STORAGE_KEY,
  compareData,
  comparisonDetail,
